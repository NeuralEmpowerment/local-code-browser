...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Bump { bump_type, commit, tag } => {
...
fn get_version_files() -> Result<Vec<VersionFile>> {
    ...
    for cargo_file in cargo_files {
        ...
        if path.exists() {
            ...
            let version = cargo_toml.package
    ...
    if Path::new(tauri_config_path).exists() {
        ...
        let version = tauri_config.version
            .and_then(|v| Version::parse(&v).ok());
...
fn check_version_sync() -> Result<()> {
    ...
    let versions: Vec<(&Version, &String)> = files.iter()
    ...
    if all_same {
        ...
        println!("{}", "❌ Version synchronization issues found!".red().bold());
        println!("{}", "=====================================".red().bold());
...
fn bump_version(bump_type: BumpType, commit: bool, tag: bool) -> Result<()> {
    ...
    let current_version = files.iter()
    ...
    if commit {
        ...
        run_command("git", &["add", "."])?;
        let commit_msg = format!("chore: bump version from {} to {}", current_version, new_version);
    ...
    if tag {
        ...
        let tag_msg = format!("Version {}: Version bump", new_version);
        run_command("git", &["tag", "-a", &tag_name, "-m", &tag_msg])?;
        println!("  ✅ Tag {} created", tag_name.green());
    }
    
    println!();
    println!("{}", "Version bump completed successfully! 🎉".green().bold());
